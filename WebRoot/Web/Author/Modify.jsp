...
<body>
	<form name="form1" action="../../Author.do" method="post">
		...
		<%
			Author model = new AuthorDao().GetModel(Integer.parseInt(request
					.getParameter("id")));
		%>
		<div class="place">
			<span>当前位置：</span>
		...
		<div class="formbody">
			...
			<ul class="forminfo">
				<li><label> 姓名:</label> <input type="text" id="Name"
					name="Name" value="<%=model.getName()%>" class="dfinput" /></li>
				<li><label> 单位:</label> <input type="text" id="Company"
					name="Company" value="<%=model.getCompany()%>" class="dfinput" />
				</li>
				<li><label> 邮箱:</label> <input type="text" id="Email"
					name="Email" value="<%=model.getEmail()%>" class="dfinput" /></li>
