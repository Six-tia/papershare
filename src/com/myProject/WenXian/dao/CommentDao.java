...
/*** 获得最大ID** @return*/
		...
		{
		 sql = "insert into Comment( UserId,PaperId,Contents,Position,PTime,Zan,AddTime,Remark) values (?,?,?,?,?,?,?,?)";
			...
			paramsList.add(model.getPosition());
			paramsList.add(model.getPTime());
			paramsList.add(model.getZan());
		...
		{
		 sql = " update  Comment set  UserId=?,PaperId=?,Contents=?,Position=?,PTime=?,Zan=?,AddTime=?,Remark=? where Id=? ";
			paramsList = new ArrayList<Object>();
						paramsList.add(model.getUserId());
			paramsList.add(model.getPaperId());
			paramsList.add(model.getContents());
		...
		public Comment GetModel(int id)
		{
			...
			Map row = res.getRows()[0];
					...
					model.setPosition((String)row.get("Position"));
					model.setPTime((String)row.get("PTime"));
					model.setZan((Integer)row.get("Zan"));
		...
		/***得到列表** @return*/
		public List<Comment> GetList(String strWhere)
		{
			...
			if (res != null && res.getRowCount() >0) {// 存在数据
			 list = new ArrayList<Comment>();//开始循环
		...
		public List<Comment> GetListByPage(int pageSize, int currentPage,String strWhere)
		{
			if(strWhere.equals("")) {
					strWhere = " 1=1 ";
			}
			List<Comment> list = null;
			sql = "select top " + pageSize + " * from Comment " ;
			String sw = " where " + strWhere + " order by id Desc ";
			sql +=" where " + strWhere ;
			if (currentPage > 1) {
					sql += " and id not in(select top "+ (pageSize * (currentPage - 1)) + " id from Comment "+ sw + " ) ";
			}
					sql += " order by id Desc ";
			Result res = bd.getResultSet(sql, paramsList);
			if (res != null && res.getRowCount() >0) {// 存在数据
		...
		public List<Comment> GetListByPage2(int pageSize, int currentPage,String strWhere)
		{
			if(strWhere.equals("")) {
					strWhere = " 1=1 ";
			}
		...
		{
			List<Comment> list = this.GetList(strWhere);
			int totalCount = 0;
