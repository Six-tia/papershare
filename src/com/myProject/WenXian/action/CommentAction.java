...
public class CommentAction extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("add")) {
			...
			model.setPosition(request.getParameter("Position"));
			model.setPTime(time);
			model.setZan(0);
		...
		if (action.equals("update")) {
			...
			model.setContents(request.getParameter("Contents"));
			model.setPosition(request.getParameter("Position"));
			model.setPTime(request.getParameter("PTime"));
			model.setZan(Integer.parseInt(request.getParameter("Zan")));
			model.setAddTime(time);
			model.setRemark("");
		...
		if (action.equals("zan")) {
			...
			model.setZan(model.getZan() + 1);

			if (dal.Update(model) > 0) {
				out.print(CommonMethod.outScript("点赞操作成功!", "back"));
			}
		}
